...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $result   = mysqli_query($conn,
        "SELECT e.*, d.name as dept_name FROM employees e
         ...
         WHERE e.cnic='$cnic' AND e.`password`='$password'");
    if (mysqli_num_rows($result) == 1) {
        $employee = mysqli_fetch_assoc($result);
        $_SESSION['employee_id']   = $employee['id'];
        $_SESSION['employee_name'] = $employee['full_name'];
        $_SESSION['employee_dept'] = $employee['dept_name'];
        header("Location: employee_dashboard.php");
        exit();
    } else {
...
<head>
  <meta charset="UTF-8">
  <title>RSKF — Employee Login</title>
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .bg-animation {
      ...
      background:linear-gradient(135deg, #0f0f0f 0%, #001a0a 50%, #0f0f0f 100%);
    }
    .bg-animation::before {
      content:''; position:absolute; top:-50%; left:-50%;
      width:200%; height:200%;
      background:radial-gradient(ellipse at center, rgba(39,174,96,0.1) 0%, transparent 60%);
    ...
    .form-group input:focus { outline:none; border-color:#27ae60; background:rgba(255,255,255,0.08); box-shadow:0 0 0 3px rgba(39,174,96,0.15); }
    ...
    .btn-login:hover { transform:translateY(-2px); box-shadow:0 8px 20px rgba(39,174,96,0.4); }

    .links { margin-top:22px; text-align:center; font-size:13px; color:rgba(255,255,255,0.3); }
    .links a { color:#27ae60; text-decoration:none; font-weight:600; }
    .links a:hover { color:#2ecc71; }
...
<div class="login-container">
  <div class="left-panel">
    ...
    <div class="role-badge">
      ...
      <p>View your profile, salary and department details</p>
    </div>
    <div class="footer">&copy; <?php echo date('Y'); ?> RSKF Group</div>
  </div>
  <div class="right-panel">
    <h2>Employee Login 👷</h2>
    <p class="subtitle">Login with your CNIC and password.</p>
    <?php if ($error): ?>
    ...
    <form method="POST">
      <div class="form-group">
        ...
        <div class="input-wrap">
          ...
          <input type="text" name="cnic" placeholder="35201-1234567-1" required autofocus>
        </div>
      </div>
      <div class="form-group">
    ...
    <div class="links">
      First time? <a href="register.php?role=employee">Set your password</a>
